...
@F:/VisualStudio/CattleFarm/.qwen/get-shit-done/references/agent-contracts.md
@F:/VisualStudio/CattleFarm/.qwen/get-shit-done/references/context-budget.md
@F:/VisualStudio/CattleFarm/.qwen/get-shit-done/references/gates.md
</required_reading>
...
```bash
_GSD_SHIM_NAME="gsd-tools.cjs"; _GSD_RUNTIME_ROOT="${RUNTIME_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"; GSD_TOOLS="${_GSD_RUNTIME_ROOT}/get-shit-done/bin/${_GSD_SHIM_NAME}"; if [ -f "$GSD_TOOLS" ]; then gsd_run() { node "$GSD_TOOLS" "$@"; }; elif [ -f "${_GSD_RUNTIME_ROOT}/.qwen/get-shit-done/bin/${_GSD_SHIM_NAME}" ]; then GSD_TOOLS="${_GSD_RUNTIME_ROOT}/.qwen/get-shit-done/bin/${_GSD_SHIM_NAME}"; gsd_run() { node "$GSD_TOOLS" "$@"; }; elif command -v gsd-tools >/dev/null 2>&1; then GSD_TOOLS="$(command -v gsd-tools)"; gsd_run() { "$GSD_TOOLS" "$@"; }; elif [ -f "F:/VisualStudio/CattleFarm/.qwen/get-shit-done/bin/${_GSD_SHIM_NAME}" ]; then GSD_TOOLS="F:/VisualStudio/CattleFarm/.qwen/get-shit-done/bin/${_GSD_SHIM_NAME}"; gsd_run() { node "$GSD_TOOLS" "$@"; }; else echo "ERROR: gsd-tools.cjs not found at $GSD_TOOLS and gsd-tools is not on PATH. Run: npx -y @opengsd/gsd-core@latest --claude --local" >&2; exit 1; fi
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @F:/VisualStudio/CattleFarm/.qwen/get-shit-done/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @F:/VisualStudio/CattleFarm/.qwen/get-shit-done/references/planner-antipatterns.md
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `F:/VisualStudio/CattleFarm/.qwen/get-shit-done/workflows/execute-plan.md` **inline**
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     prompt="
       ...
       <execution_context>
       @F:/VisualStudio/CattleFarm/.qwen/get-shit-done/workflows/execute-plan.md
       @F:/VisualStudio/CattleFarm/.qwen/get-shit-done/templates/summary.md
       @F:/VisualStudio/CattleFarm/.qwen/get-shit-done/references/checkpoints.md
       @F:/VisualStudio/CattleFarm/.qwen/get-shit-done/references/tdd.md
       @F:/VisualStudio/CattleFarm/.qwen/get-shit-done/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@F:/VisualStudio/CattleFarm/.qwen/get-shit-done/references/executor-examples.md'}
       </execution_context>
       ...
       ` : ''}
       - ${PROJECT_ROOT}/QWEN.md (Project instructions, if exists — follow project-specific guidelines and coding conventions)
       - ${PROJECT_ROOT}/.qwen/skills/ or ${PROJECT_ROOT}/.agents/skills/ (Project skills, if either exists — list skills, read SKILL.md for each, follow relevant rules during implementation)
       </files_to_read>
...
Read and follow `F:/VisualStudio/CattleFarm/.qwen/get-shit-done/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
