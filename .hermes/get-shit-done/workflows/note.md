...
- **Global scope**: `F:/VisualStudio/CattleFarm/.hermes/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `F:/VisualStudio/CattleFarm/.hermes/notes/`)
...
2. Glob `F:/VisualStudio/CattleFarm/.hermes/notes/*.md` (if directory exists) — global notes
...
Global (F:/VisualStudio/CattleFarm/.hermes/notes/):
...
2. **No `.planning/`**: Falls back to global `F:/VisualStudio/CattleFarm/.hermes/notes/` — works in any directory
